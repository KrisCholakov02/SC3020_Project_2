...
class GUI1():
    ...
    def initialise_GUI1(self):
        ...
        frame1_layout = [
            [sg.Frame(layout=col1, title='', pad=5, expand_x=True, vertical_alignment="top", border_width=0,
                      element_justification='center'),
             sg.Frame(layout=col2, title='', pad=5, expand_x=True, element_justification='center')]
        ]

        layout = [
            [sg.T('CZ4031 Query Execution Plan Annotator', font='_ 16', justification='l', background_color='darkblue',
                  text_color='papaya whip', expand_x=True)],
            [sg.Frame('Control Settings', frame0_layout, pad=5, expand_x=True, element_justification='right')],
            [sg.Frame('Step 1: Login and Provide Query', frame1_layout, pad=5, expand_x=True,
                      element_justification='center')],
        ...
        layout2 = [
            [sg.Column(layout, scrollable=True, expand_y=True, expand_x=True, size=(1500, 3000),
                       element_justification='center')]
        ]

        # Create the Window
        window = sg.Window('CZ4031 Database Project 2 GUI', layout2, size=(700, 990), resizable=True,
        ...
        while True:
            ...
            if event == 'Submit':  # when user clicks submit button
                ...
                sg.SystemTray.notify('Query Submitted to PostgreSQL!', 'Please wait for results!',
                                     display_duration_in_ms=1000, fade_in_duration=0)
...
class GUI2():
    ...
    def initialise_GUI2(self, query, query_plan, query_plan_costs, query_plan_scenarios):
        ...
        frame0_layout = [
            [sg.Frame(layout=col1, title='', vertical_alignment="top", border_width=0, element_justification='center'),
             sg.Frame(layout=col2, title='', border_width=0, element_justification='center')]
        ]
        ...
        color_list = ['dark orange', 'orchid1', 'turquoise3', 'medium sea green', 'gold4', 'chartreuse3', 'rosy brown',
                      'deep sky blue', 'DarkOliveGreen4', 'coral1', 'DarkCyan', 'salmon1', 'wheat4', 'DarkMagenta',
                      'DarkOrange3', 'cornflower blue', 'GreenYellow', 'plum3', 'sienna3', 'medium violet red',
                      'MistyRose3', 'OrangeRed3']
        colors = {}
        ...
        # Query Annotations
        annotation_layout = [
            [sg.Text('Query Plan Annotation and Statistics', font=(font, 12), justification='left',
                     background_color='darkblue', text_color='papaya whip', expand_x=True)],
        ]
        annotation_layout += [[sg.Text(f'{i.annotation_text} ', size=(60, None), text_color=colors[i])] for i in
                              optimal_qep_nodes]

        # Input query text
        query_text_layout = [
            [sg.Text('Input Query', font=(font, 12), justification='left', background_color='darkblue',
                     text_color='papaya whip', expand_x=True)],
        ]
        ...
        # frame 2 layout
        frame2_layout = [
            [sg.Text('Query Plan Operator Tree', font=(font, 12), justification='left', background_color='darkblue',
                     text_color='papaya whip', expand_x=True)],
            [sg.Frame(layout=canvas_column_layout, title='', pad=5, expand_x=True, vertical_alignment="top",
                      border_width=0, element_justification='left')],
            [sg.Text(' ', font=(font, 12))],  # just empty space to space it out
            [
                sg.Frame(layout=query_text_layout, title='', pad=5, expand_x=True, vertical_alignment="top",
                         border_width=0, element_justification='left'),
                sg.Frame(layout=annotation_layout, title='', pad=5, expand_x=True, vertical_alignment="top",
                         border_width=0, element_justification='left')
            ],
        ]
        ...
        # frame 3 layout
        frame3_layout = [
            [sg.Text('Cost of current optimal Query Plan', font=(font, 12), justification='left',
                     background_color='darkblue', text_color='papaya whip', expand_x=True)],
            [sg.Text(optimal_cost)],
            [sg.Text('Cost of alternative Query Plans', font=(font, 12), justification='left',
                     background_color='darkblue', text_color='papaya whip', expand_x=True)],
            [sg.Frame(layout=aqp_layout, title='', pad=5, expand_x=True, vertical_alignment="top", border_width=0,
                      element_justification='left')]
        ]

        # window layout
        layout = [
            [sg.Text('CZ4031 Query Execution Plan Annotator', font='_ 16', justification='l',
                     background_color='darkblue', text_color='papaya whip', expand_x=True)],
            [sg.Frame('Control Settings', frame0_layout, pad=5, expand_x=True, element_justification='right')],
            [sg.Frame('Step 2: Display QEP and Output', frame2_layout, pad=5, expand_x=True,
        ...
        # Nesting the above into a column so as to allow vertical scrolling
        layout2 = [
            [sg.Column(layout, scrollable=True, expand_y=True, expand_x=True, size=(1100, 2400),
                       element_justification='center')]
        ]

        # Create the Window
        window = sg.Window('CZ4031 Database Project 2 GUI', layout2, resizable=True,
                           element_justification='c').Finalize()
        # window.Maximize()
        ...
        for element in optimal_qep_nodes:
            for child in element.pointers:
                line = canvas.TKCanvas.create_line(child.center[0], child.center[1] - annotation.BOX_H / 2,
                                                   element.center[0],
